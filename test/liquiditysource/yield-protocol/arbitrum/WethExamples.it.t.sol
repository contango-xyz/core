...
contract YieldArbitrumWethExamplesUSDCETHTest is
    WithYieldFixtures(constants.yUSDCETH2212, constants.FYUSDC2212, constants.FYETH2212)
{
    ...
    function testIncreasePosition() public {
        uint256 initialQuantity = 3000e6;
        ...
        ModifyCostResult memory increasingCostResult = contangoQuoter.modifyCostForPosition(
            ModifyCostParams(positionId, int256(quantity), 1 ether, collateralSlippage)
        );
    ...
    function testIncreaseAndCollateralisePositionAtMax() public {
        ...
        int256 amountToUseInAddCollateralCall = ((result.debtDelta - result.financingCost).absi() * 0.999e18) / 1e18;
        ...
        assertApproxEqAbsDecimal(
            trader.balance, collateral - result.collateralUsed.abs(), 0.02 ether, quoteDecimals, "trader balance"
        );
        assertEq(WETH.balanceOf(address(contango)), 0);
    }
    ...
    function testDecreasePosition() public {
        ...
        // unwrap is not necessary since the no settlement nor excessive quote will be generated
        // adding it wouldn't break the code but would incur in unnecessary gas expenditure
    ...
    function testFullyClosePosition() public {
        ...
        assertApproxEqAbs(trader.balance, openPositionResult.collateralUsed.abs(), 0.02 ether, "trader balance");
        assertEq(WETH.balanceOf(address(contango)), 0);
    }

    function testDecreasePositionWithExcessQuote() public {
        uint256 initialQuantity = 2000e6;
        ...
        (PositionId positionId,) = _openPosition(initialQuantity, 2 ether);
        ...
        assertApproxEqAbsDecimal(
            trader.balance, result.collateralUsed.abs(), 0.001 ether, quoteDecimals, "trader balance"
        );
        assertEq(WETH.balanceOf(address(contango)), 0);
    }
    ...
    function testRemoveCollateral() public {
        ...
        assertEq(contango.position(positionId).openQuantity, quantity);
        assertEq(trader.balance, result.collateralUsed.abs());
        assertEq(WETH.balanceOf(address(contango)), 0);
    }
}

contract YieldArbitrumWethExamplesETHUSDCTest is
